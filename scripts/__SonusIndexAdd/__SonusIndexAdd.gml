function __SonusIndexAdd(_filePath, _group = undefined, _preload = false, _async = false) {
	...
	if (!is_undefined(_group)) {
		if (!__SonusGroupExists(_group)) {
			__SonusGroupAdd(_group);
		}
		__SonusGroupAddSound(_snd, _group);
	}
	return _snd;
}